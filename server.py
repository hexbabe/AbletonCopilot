...
from ableton.v2.base import lazy_attribute
...
class AbletonCopilotServer:
    ...
    def log_message(self, message: str) -> None:
        ...
        try:
            ...
            print(f"Error writing to log file: {e}")  # Last resort logging
    
    def start_server(self) -> None:
        """Initialize and start the socket server in a separate thread."""
        try:
            self.server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            # Add socket reuse to prevent "address already in use" during development
        ...
        while True:
            try:
                ...
                if data == "set_tempo":
                    ...
                    self.song.tempo = 69.0
                    self.log_message(f"Tempo changed from {old_tempo} to 69.0")
                    client.send(b"Tempo set to 69 BPM")
